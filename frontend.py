...
def search_entry_command():
    entries_field.delete(0, END)
    for row in backend.search_entry(title_input.get(), author_input.get(), year_input.get(), isbn_input.get()):
        entries_field.insert(END, row)

def add_entry_command():
    backend.add_entry(title_input.get(), author_input.get(), year_input.get(), isbn_input.get())
    entries_field.delete(0, END)
    entries_field.insert(END, (title_input.get(), author_input.get(), year_input.get(), isbn_input.get()))

def update_selected_command():
    backend.update_selected(selected_row[0], title_entry.get(), author_entry.get(), year_entry.get(), isbn_entry.get())


def delete_selected_command():
    backend.delete_selected(selected_row[0])
